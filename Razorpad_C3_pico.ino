...
BleKeyboard BleKeyboard("Razorpad C3 Pico", "Razorbac", 69);
...
int Button7 = 6; //F6 / ZOOM +
int Button8 = 8; //F7 / ZOOM -
...
void setup() {
  // Serial
  Serial.begin(115200);
  Serial.println("Avvio RazorPad C3 Pico");
  // bleKeyboard
  BleKeyboard.begin();
  ...
  Serial.println("RazorPad C3 Pico Avviato");
}


void batteryLevel() {
  int sensorValue = analogRead(ADC);
  float voltage = sensorValue * (5.52 / 4095);
